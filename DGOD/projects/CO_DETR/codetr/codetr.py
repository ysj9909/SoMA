...
class CoDETR(BaseDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if self.pego_reg:
            ...
            num_groups = self.backbone.peft_config.group
            
            for module_name in self.backbone.lora_modules:
                module = reduce(getattr, ('model.' + module_name).split("."), self.backbone)
                # module = getattr(self.backbone.model, 'model.' + module_name)
                if self.backbone.peft_config.sora_adapted:
                    weight = module.truncated_weight
                    print("*"*2000)
                else:
                    weight = module.weight
                for i in range(num_groups):
                    ab = torch.mm(module.lora_B[i].weight, module.lora_A[i].weight)
                    loss_pego += torch.abs(torch.mm(weight, ab.T)).sum()
                    
                    for j in range(i+1, num_groups):
                        ab_j = torch.mm(module.lora_B[j].weight, module.lora_A[j].weight)
                        loss_pego += torch.abs(torch.mm(ab, ab_j.T)).sum()
            
            loss_pego *= 1e-3
            loss_pego = {'pego.reg_loss': loss_pego}
            losses.update(loss_pego)
